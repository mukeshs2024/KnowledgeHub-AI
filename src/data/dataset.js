...
export function generateDocuments(rows) {
  return rows.map((row, index) => ({
    ...
    text: Object.entries(row)
      ...
      .map(([field, value]) => `${prettifyName(field)} is ${stringifyValue(value)}.`)
      .join('\n'),
    metadata: Object.fromEntries(
      Object.entries(row)
        .filter(([, value]) => !isEmpty(value))
        .map(([field, value]) => [field, stringifyValue(value)])
    ),
    row,
  }));
}
...
export async function parseUploadedFile(file) {
  ...
  if (extension === 'csv') {
    ...
    return dataset_analysis_agent({ rows: cleanRows(parseCsv(text)), fileName: file.name, fileType: 'CSV' });
  }

  if (extension === 'json') {
    const text = await file.text();
    const json = JSON.parse(text);
    return dataset_analysis_agent({ rows: cleanRows(findJsonRecords(json)), fileName: file.name, fileType: 'JSON' });
  }
  ...
  if (extension === 'xlsx') {
    ...
    return dataset_analysis_agent({ rows: cleanRows(rows), fileName: file.name, fileType: 'Excel' });
  }
...
export function answerQuestion(dataset, question) {
  ...
  if (/how many|count|total records|number of/i.test(question)) {
    return `The active ${dataset.metadata.datasetType.toLowerCase()} has ${dataset.rows.length.toLocaleString()} records and ${dataset.columns.length.toLocaleString()} detected fields.`;
  }
  ...
  if (numericColumn && /average|avg|mean/i.test(normalized)) {
    ...
    return `Average ${numericColumn.label} is ${average.toLocaleString(undefined, { maximumFractionDigits: 2 })} across ${numbers.length} records.`;
  }
  ...
  if (numericColumn && /highest|max|maximum|top|largest|most/i.test(normalized)) {
    ...
    return ranked.length
      ? `Highest ${numericColumn.label}: ${ranked.map((item) => `${item.value.toLocaleString()} (${formatRow(item.row, dataset.columns)})`).join('\n')}`
      : `I could not find numeric values for ${numericColumn.label}.`;
  }
  ...
  if (numericColumn && /lowest|min|minimum|smallest|least/i.test(normalized)) {
    const ranked = dataset.rows
      ...
      .slice(0, 1);
    return ranked.length ? `Lowest ${numericColumn.label}: ${ranked[0].value.toLocaleString()} (${formatRow(ranked[0].row, dataset.columns)})` : `I could not find numeric values for ${numericColumn.label}.`;
  }

  const matches = retrieveDocuments(dataset, question);
  if (matches.length) {
    return `I found ${matches.length} relevant record${matches.length > 1 ? 's' : ''}:\n${matches.map((match, index) => `${index + 1}. ${formatRow(match.row, dataset.columns)}`).join('\n')}`;
  }

  return `I could not find an exact match in this dataset. Try asking about one of these fields: ${dataset.columns.slice(0, 8).map((column) => column.label).join(', ')}.`;
}

export const defaultDataset = dataset_analysis_agent({
  rows: cleanRows(parseCsv(csvText)),
  fileName: 'Rag_Project_Dataset.csv',
  fileType: 'CSV',
});
